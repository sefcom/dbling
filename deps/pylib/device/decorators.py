...
#                              'android_testrunner'))
from third_party.android_testrunner import errors as old_errors
...
def _TimeoutRetryWrapper(f, timeout_func, retries_func, pass_values=False):
  ...
  def TimeoutRetryWrapper(*args, **kwargs):
    ...
    try:
      ...
      raise device_errors.CommandTimeoutError(str(e))(None).with_traceback((
        sys.exc_info()[2]))
    except old_errors.DeviceUnresponsiveError as e:
      raise device_errors.DeviceUnreachableError(str(e))(None).with_traceback((
        sys.exc_info()[2]))
