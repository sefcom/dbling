...
import errno
import os, pickle, random, re, resource, select, shutil, signal
import io as StringIO
import socket, struct, subprocess, time, textwrap
...
def run_parallel(commands, timeout=None, ignore_status=False,
    ...
    """
    bg_jobs = []
    for (command, nickname) in itertools.zip_longest(commands, nicknames):
...
def compare_versions(ver1, ver2):
    ...
    while len(ax) > 0 and len(ay) > 0:
        ...
        if cx.zfill(maxlen) < cy.zfill(maxlen):
            c = -1
        elif cx.zfill(maxlen) == cy.zfill(maxlen):
            c = 0
        elif cx.zfill(maxlen) > cy.zfill(maxlen):
            c = 1
        if c != 0:
